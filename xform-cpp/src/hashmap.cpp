...
HashMap* hm_new(void) {
    HashMap *hm = static_cast<HashMap*>(malloc(sizeof(HashMap)));
    if (!hm) return NULL;
    hm->buckets = static_cast<Entry**>(calloc(INITIAL_CAPACITY, sizeof(Entry*)));
    if (!hm->buckets) {
        free(hm);
        return NULL;
    }
...
HMEntry* hm_entries(HashMap *hm, size_t *count) {
    ...
    *count = hm->size;
    ...
    HMEntry *entries = static_cast<HMEntry*>(malloc(hm->size * sizeof(HMEntry)));
