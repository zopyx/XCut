...
FIXTURES = ROOT / "tests" / "fixtures_22_errors"
ENABLED_LANGS = {
    s.strip().lower()
...
def _cases():
    return sorted(
        ...
        if p.is_dir() and (p / "expected_error.txt").exists()
    )
...
def test_xform_errors_22(lang: str, case: Path) -> None:
    result = subprocess.run(
        _cmd(lang, case / "transform.xform", case / "input.xml"),
        capture_output=True,
        text=True,
    )
    expected = (case / "expected_error.txt").read_text(encoding="utf-8").strip()
